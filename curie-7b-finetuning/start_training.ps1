...
# Download finetune_llama_v2.py
Invoke-WebRequest -Uri "https://gist.githubusercontent.com/younesbelkada/9f7f75c94bdc1981c8ca5cc937d4a4da/raw/6f027020ed295447a2c476c610b99f2441b45e39/finetune_llama_v2.py" -OutFile "finetune_llama_v2.py"

# Run finetune_llama_v2.py
...
# Run merge_peft_adapters.py
python merge_peft_adapters.py --base_model_name_or_path meta-llama/Llama-2-7b-hf --peft_model_path $adaptername --output_dir ./models/merged/ --device auto
...
# Quantize model
./build/Release/quantize.exe ./merged-model/ $quant_type ./gguf/merged-model.q4_k_m.gguf.bin

# Convert model to f16
python .\llama.cpp\convert.py ./merged-model/ --outtype f16 --outfile ./gguf/merged-model.fp16.bin

# Run main.exe
.\llama.cpp\build\bin\Release\main.exe .\gguf\merged-model.q4_k_m.fp16.bin q4_k_m -n 128
